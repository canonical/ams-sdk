...
func (res *InstanceResources) ToApplicationResources() ApplicationResources {
	...
	if res.Memory > 0 {
		resources.Memory = shared.GetByteSizeString(res.Memory, 0)
	}
	if res.DiskSize > 0 {
		resources.DiskSize = shared.GetByteSizeString(res.DiskSize, 0)
	}
	if res.GPUSlots > 0 {
		resources.GPUSlots = res.GPUSlots
	}
	if res.VPUSlots > 0 {
		resources.VPUSlots = res.VPUSlots
	}
	return resources
}
