...
func (c *clientImpl) DeleteImageByIDOrName(id string, force bool, imgType api.ImageType) (client.Operation, error) {
	if len(id) == 0 {
	...
	op, _, err := c.QueryOperation("DELETE", client.APIPath("images", id), params, nil, bytes.NewReader(b), "")
	return op, err
}
...
func (c *clientImpl) RetrieveImageByIDOrName(id string, imgType api.ImageType) (*api.Image, string, error) {
	if len(id) == 0 {
		return nil, "", errs.NewInvalidArgument("id")
	}
	i := &api.Image{}
	...
	etag, err := c.QueryStruct("GET", client.APIPath("images", id), params, nil, nil, "", i)
	return i, etag, err
}
